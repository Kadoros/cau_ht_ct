...
class FNOBlock(nn.Module):
    def __init__(
        ...
        self.norm = nn.BatchNorm2d(channels)  # GroupNorm -> BatchNorm2d
...
class HybridFNONet(nn.Module):
    def __init__(
        ...
        self.lifting = nn.Sequential(
            nn.Conv2d(
                3, channels, kernel_size=3, padding=1, bias=False
            ),  # 1채널 -> 3채널 (CoordConv)
        ...
        # 차원 병목 해결: 채널 수 2배 확장 및 공간 크기 축소 (14x14)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # [핵심] CoordConv: x, y 좌표 텐서를 추가하여 '좌/우' 장기를 명확히 구분하게 함
