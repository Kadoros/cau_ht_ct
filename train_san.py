...
class MedDataset(Dataset):
    ...
    def __init__(self, image_dir: str, label_csv: str, augment: bool = False):
        ...
        print(f"    Label range: {self.labels.min().item()} ~ {self.labels.max().item()}")
    ...
    def _augment(self, img: torch.Tensor) -> torch.Tensor:
        ...
        if torch.rand(()) < 0.55:
            ...
            img = _affine_tensor(img, angle_deg=angle, translate_px=(tx, ty), scale=scale)
...
def _affine_tensor(
    ...
    grid = F.affine_grid(theta.unsqueeze(0), img.unsqueeze(0).shape, align_corners=False)
    out = F.grid_sample(
        img.unsqueeze(0),
        grid,
        align_corners=False,
...
def stratified_split_indices(labels: torch.Tensor, val_per_class: int = 10, seed: int = 42) -> Tuple[List[int], List[int]]:
...
def pretrain(
    ...
    loader = DataLoader(combined, batch_size=batch_size, shuffle=True, num_workers=0, drop_last=False)
...
class TaskSampler:
    ...
    def __init__(self, dataset: Dataset, n_way: int = 11, k_shot: int = 5, q_query: int = 10):
    ...
    def sample_task(self, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
...
class CrossDomainTaskSampler:
    ...
    def __init__(self, support_dataset: Dataset, query_dataset: Dataset, n_way: int = 11, k_shot: int = 5, q_query: int = 10):
        ...
        self.classes = sorted(set(self.support_indices.keys()) & set(self.query_indices.keys()))
    ...
    def sample_task(self, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for cls in chosen_classes:
            ...
            s_chosen = np.random.choice(s_idx, size=self.k_shot, replace=len(s_idx) < self.k_shot)
            q_chosen = np.random.choice(q_idx, size=self.q_query, replace=len(q_idx) < self.q_query)
...
def _forward_with_weights(model: nn.Module, x: torch.Tensor, weights: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        for _step in range(inner_steps):
            ...
            for (name, weight), grad in zip(fast_weights.items(), grads):
                if grad is None:
                    ...
                    updated[name] = (weight - inner_lr * grad).detach().clone().requires_grad_(True)
...
def meta_train(
    ...
    samplers = [
        CrossDomainTaskSampler(axial_dataset, coronal_dataset, n_way=11, k_shot=k_shot, q_query=q_query),
        CrossDomainTaskSampler(coronal_dataset, axial_dataset, n_way=11, k_shot=k_shot, q_query=q_query),
    ...
    meta_optimizer = torch.optim.AdamW(model.parameters(), lr=meta_lr, weight_decay=1e-5)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(meta_optimizer, T_max=max(1, meta_epochs))
    model.to(device)
...
def finetune_l2sp(
    ...
    theta_meta = {name: param.detach().clone() for name, param in model.named_parameters()}
    train_loader = DataLoader(sagittal_dataset, batch_size=batch_size, shuffle=True, num_workers=0, drop_last=False)
    ...
    for epoch in range(epochs):
        ...
        if val_datasets is not None and ((epoch + 1) % eval_every == 0 or epoch == epochs - 1):
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            if val_datasets is not None:
                print(f"  FT Epoch [{epoch+1:3d}/{epochs}] loss={avg_loss:.4f} acc={acc:.1f}% best_val={best_metric:.4f}")
            else:
                print(f"  FT Epoch [{epoch+1:3d}/{epochs}] loss={avg_loss:.4f} acc={acc:.1f}%")
...
def l2sp_penalty(model: nn.Module, theta_meta: Dict[str, torch.Tensor], device: torch.device) -> torch.Tensor:
...
# 4. Evaluation Helper
# ══════════════════════════════════════════════════════════
def evaluate(model: nn.Module, dataset: Dataset, device: torch.device, batch_size: int = 128) -> float:
    ...
    return float(f1_score(all_lbls, all_preds, labels=list(range(11)), average="macro", zero_division=0))
...
def search_lambda(
    ...
    for lam in lambdas:
        ...
        scores = evaluate_all(model, axial_eval, coronal_eval, sagittal_val, device, verbose=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Few-Shot Domain Generalization Training")
    parser.add_argument("--train_dir", required=True, help="Root train dir containing axial/, coronal/, sagittal/, label/")
    ...
    parser.add_argument("--skip_meta", action="store_true", help="Skip FOMAML if you need a faster ablation")

    parser.add_argument("--finetune_epochs", type=int, default=160)
    parser.add_argument("--finetune_lr", type=float, default=2e-4)
    parser.add_argument("--lambda_val", type=float, default=0.05, help="L2-SP regularization weight")
    parser.add_argument("--search_lambda", action="store_true", help="Run lambda grid search using stratified Sagittal validation")
...
def main() -> None:
    ...
    t = args.train_dir
    print("\nLoading datasets...")
    axial_train = load_domain(os.path.join(t, "axial"), os.path.join(t, "label", "axial.csv"), augment=True)
    coronal_train = load_domain(os.path.join(t, "coronal"), os.path.join(t, "label", "coronal.csv"), augment=True)
    sagittal_full_train = load_domain(os.path.join(t, "sagittal"), os.path.join(t, "label", "sagittal.csv"), augment=True)

    axial_eval = load_domain(os.path.join(t, "axial"), os.path.join(t, "label", "axial.csv"), augment=False)
    coronal_eval = load_domain(os.path.join(t, "coronal"), os.path.join(t, "label", "coronal.csv"), augment=False)
    sagittal_eval = load_domain(os.path.join(t, "sagittal"), os.path.join(t, "label", "sagittal.csv"), augment=False)
    ...
    if args.search_lambda:
        train_idx, val_idx = stratified_split_indices(sagittal_eval.labels, val_per_class=args.val_per_class, seed=args.seed)
    ...
    scores = evaluate_all(model, axial_eval, coronal_eval, sagittal_eval, device, verbose=True)
